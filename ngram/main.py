...
def get_frequency_for_n_number_of_consecutive_words(
    n: int, file: UploadFile = File(...)
):
...
def get_upto_x_number_of_predictions_after_a_given_text(
    n: int, x: int, search_text: str, file: UploadFile = File(...)
):
    service = NGramParser(n=n, file=file)
