...
class NGramParser:
    ...
    def get_frequency_and_predictions(self) -> dict:
        self.run()
        return {
            "Frequencies": self.frequency_map,
            "Predictions": self.prediction_map
        }
    ...
    def get_predictions(self, x: int, text: str) -> dict:
        self.run()
        result = {
            f'candidate completions for [{text.lower()}]': {}
        }
        ...
        while len(self.words) >= self.n:
            ...
            for i in range(start_index, end_index):
                ...
                if i != len(self.words)-1:
                    self.prediction_map[current_string].append(self.words[i+1].lower())
